...
function computeRealCSDDeseasonalized(monthlyColl, study, bandName, scale, onDone) {
  var valsFC = ee.FeatureCollection(monthlyColl.map(function(img) {
    var v = img.reduceRegion({reducer: ee.Reducer.mean(), geometry: study, scale: scale, maxPixels: 1e9}).get(bandName);
    return ee.Feature(null, {v: v, t: img.get('system:time_start')});
  })).sort('t');
  ...
  valsFC.evaluate(function(fcResult, err) {
    ...
    var result = jsNodeStatsFixed(seriesTV);
    onDone(result);
  });
}
...
function jsNodeStatsFixed(seriesTV) {
  var clean = (seriesTV||[]).filter(function(s){ return s && s.v!==null && s.v!==undefined && !isNaN(s.v); });
  var n = clean.length;
  if (n < 4) return {realAC1:null,varFirstHalf:null,varSecondHalf:null,varTrendRatio:null,skewness:null,nValidMonths:n,deseasonalized:false};
  var valuesOnly;
  var wasDeseasonalized;
  if (n >= 24) {
    ...
    // Window too short for calendar months to repeat - deseasonalizing
    // would be degenerate (see caught bug above). Fall back to the plain
    // computation, same as the tool's original behavior for this case.
    valuesOnly = clean.map(function(s){ return s.v; });
    wasDeseasonalized = false;
  }
  if (valuesOnly.length < 4) return {realAC1:null,varFirstHalf:null,varSecondHalf:null,varTrendRatio:null,skewness:null,nValidMonths:valuesOnly.length,deseasonalized:wasDeseasonalized};
  var resid = jsLinearDetrendResiduals(valuesOnly);
  var ac1 = jsLag1AC1(resid);
  var vh = jsVarianceHalves(resid);
  var skew = jsSkewness(resid);
  return {realAC1:ac1, varFirstHalf:vh.varFirst, varSecondHalf:vh.varSecond, varTrendRatio:vh.ratio, skewness:skew, nValidMonths:valuesOnly.length, deseasonalized:wasDeseasonalized};
}
...
function extractSpatialAC1(corrObj) {
  if(!corrObj || typeof corrObj!=='object') return null;
  ...
  for(var k in corrObj){
    ...
    if(typeof corrObj[k]==='number'&&!isNaN(corrObj[k])) return corrObj[k];
  }
  return null;
}
...
panel.add(lbl('STEMGeoHS Marine v10.149',12,'#ffffff','#1a4a2a',true));
...
function classifyToolkitConfidence(tally){
  if(!tally.primaryAvailable){
    if(tally.supportAvail===0) return {label:'NO INDICATORS AVAILABLE', level:'none'};
    if(tally.supportAgree>0) return {label:'LOW (AC1 unavailable - only weaker supporting indicators to go on: '+
      tally.supportAgree+'/'+tally.supportAvail+' agree)', level:'low'};
    return {label:'LOW (AC1 unavailable, no supporting indicators agree either)', level:'low'};
  }
  if(tally.primaryAgrees){
    if(tally.supportAvail>0 && tally.supportAgree===tally.supportAvail) return {label:'HIGH - primary indicator (AC1) rising, corroborated by ALL '+tally.supportAvail+' supporting indicator(s)', level:'high'};
    if(tally.supportAgree>0) return {label:'HIGH - primary indicator (AC1) rising, corroborated by '+tally.supportAgree+' of '+tally.supportAvail+' supporting indicator(s)', level:'high'};
    // v10.91 FIX: this branch fires when AC1 IS rising but the supporting
    // indicators are available and NOT agreeing (e.g. variance is flat or
    // FALLING). The old wording ("no supporting indicator corroborates it
    // yet") wrongly implied pending/weak evidence. Per Dakos et al. 2012,
    // AC1 rising while variance falls is a documented, expected pattern
    // (their Fig. 2c "decreasing sensitivity" and Fig. 4 "freezing" cases)
    // - autocorrelation "remains solely dependent on the dominant
    // eigenvalue" and rises "regardless of the responsiveness of the
    // ecosystem" to noise. Variance disagreeing does NOT weaken AC1's
    // signal; it just means variance isn't a useful witness at this site.
    if(tally.supportAvail>0) return {label:'MODERATE - primary indicator (AC1) rising; supporting indicator(s) flat/falling. Per Dakos et al. 2012 this does NOT weaken the AC1 signal - variance is documented to sometimes fall even near a genuine transition (their Fig. 2c, Fig. 4). AC1 is treated as standing on its own here.', level:'moderate'};
    return {label:'MODERATE - primary indicator (AC1) rising alone, no supporting data available (Dakos et al. 2012: AC1 alone is a relatively robust signal)', level:'moderate'};
  }
  // AC1 did NOT rise - per Dakos et al., this matters more than any
  // supporting indicator rising, since variance specifically is known to
  // be unreliable (can rise OR fall near a real transition).
  if(tally.supportAgree>0) return {label:'LOW-MODERATE - primary indicator (AC1) did NOT rise, even though '+
    tally.supportAgree+' supporting indicator(s) did. Dakos et al. 2012: variance-type indicators can rise OR fall near a real transition, so this is weaker evidence than an AC1 rise would be.', level:'low-moderate'};
  return {label:'LOW - primary indicator (AC1) did not rise, and no supporting indicators rose either', level:'low'};
}
      ...
      function permFinish(){
        try {
          ...
          try {
            ...
            var monthsUsedForClim = Object.keys(climatology).length;
            ...
            deseasonLines.push('months with any data, ~1-2 samples per month at typical window lengths - a');
            deseasonLines.push('genuinely noisy estimate, not a robust multi-decade seasonal average. Treat as');
            deseasonLines.push('an exploratory comparison, not a validated replacement for the raw AC1 above.');
  ...
  computeRealCSDDeseasonalized(ctrlBeforeColl, ctrlStudy, 'sst', 4000, function(cbRes){
    computeRealCSDDeseasonalized(ctrlAfterColl, ctrlStudy, 'sst', 4000, function(caRes){
     try {
      ...
      var detailRegionalConflict = studyAC1Rose && vTitle.indexOf('MORE STABLE THAN CONTROL')>=0;
      var detailRegionalLine = detailRegionalConflict ?
        'Regional context: CONFLICTS WITH AC1 - the paragraph below reads the variance data\n'+
        'only and says "study more stable," but AC1 (the primary indicator, \u0394='+nad(sDAC1)+') rose\n'+
        'at the study site. Once the full spatial toolkit finishes above, the headline will\n'+
        'likely say REGIONAL or SIGNAL PRESENT, not a positive/protective result - do not\n'+
        'treat the paragraph below as confirmed on its own.\n\n'+verdict :
      ...
      // ============================================================
      var skewDisplay = (a.skew!==null&&a.skew!==undefined)?
        ((a.skew>0?'+':'')+a.skew.toFixed(2)+(b.skew!==null&&b.skew!==undefined?' (was '+(b.skew>0?'+':'')+b.skew.toFixed(2)+')':'')) : 'n/a';
      ...
      function renderToolkit(indicators, spatialPending){
        ...
        else confidence=classifyToolkitConfidence(tally).label;
          ...
          rSpatialAfter.evaluate(function(spAfter, spAfterErr){
           try {
            ...
            try {
              ...
              var sAC1Before = spatialAvailable?extractSpatialAC1(spBefore.corr):null;
              var sAC1After  = spatialAvailable?extractSpatialAC1(spAfter.corr):null;
              ...
              var fullIndicators = buildTemporalIndicators().concat([
                ...
                {name:'Spatial Autocorrelation (study)', scored:true, primary:false,
                  available:dSpatialAC1!==null,
                  agrees:dSpatialAC1!==null&&dSpatialAC1>0.05,
                  display:dSpatialAC1!==null?((dSpatialAC1>0?'+':'')+dSpatialAC1.toFixed(3)):'n/a (region too small/sparse)'},
              ...
              var conf = classifyToolkitConfidence(fullTally);
              ...
              var regionalContextConflict = fullTally.primaryAgrees && (vTitle.indexOf('MORE STABLE THAN CONTROL')>=0);
              var regionalContextLine = regionalContextConflict ?
                'Regional context: CONFLICTS WITH AC1 - the variance-only read says "study more stable," but AC1 (primary indicator) rose almost identically at both sites, which is why the headline above says REGIONAL. Do not read this as a positive/protective finding when it contradicts the AC1-based headline.' :
                'Regional context: '+vTitle;
              csdCompareVerdictV.setValue(combinedTitle+'\nAC1 (primary): '+(fullTally.primaryAvailable?(fullTally.primaryAgrees?'RISING':'not rising'):'n/a')+
    ...
    function finishAnalysis(){
      try {
        ...
        for(var wi=0;wi<windowLengths.length;wi++){
          ...
          var scheffer = (sDVar!==null&&sDVar>0.01&&sDAC1!==null&&sDAC1>0.01);
          var ac1Rose = (sDAC1!==null&&sDAC1>0.01);
          var varRose = (sDVar!==null&&sDVar>0.01);
          ...
          else if(!studyVarRose&&div!==null&&div>THRESH) verdict='MARGINAL LOCAL';
      ...
      try {
        ...
        function s7eFinish(){
          try {
            ...
            var studySignal = (dAC1_study!==null&&dAC1_study>0.01) || (dVar_study!==null&&dVar_study>0.15);
            var refSignal = (dAC1_ref!==null&&dAC1_ref>0.01) || (dVar_ref!==null&&dVar_ref>0.15);
            ...
            lines.push('Decision rule: study signal (\u0394AC1>0.01 OR \u0394Var>0.15x) AND reference signal, same thresholds -> classify per STEP 3\'s LOCAL/GLOBAL/ANOMALOUS/NO SIGNAL pattern.');
...
// STARTUP
print('STEMGeoHS Marine v10.149 -- READY');
print('');
